...
<div style="max-width:900px;margin:0 auto">
  ...
  <div class="card" style="margin-bottom:24px;display:flex;align-items:center;justify-content:space-between;gap:16px;flex-wrap:wrap">
    <div>
      ...
      <div style="display:flex;align-items:center;gap:12px">
        <span style="font-size:28px"><?= PLANS[$user['plan']]['icone'] ?? '🎒' ?></span>
        <div>
  ...
  <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:20px;margin-bottom:32px">
    ...
    <div style="background:white;border-radius:20px;padding:28px 24px;border:2px solid <?= $isActive ? 'var(--primary)' : (($plan['populaire']??false) ? 'var(--gold)' : 'var(--gris-200)') ?>;position:relative;transition:all .2s" onmouseover="this.style.boxShadow='var(--shadow-lg)';this.style.transform='translateY(-3px)'" onmouseout="this.style.boxShadow='';this.style.transform=''">
      <?php if ($isActive): ?>
        <div style="position:absolute;top:-12px;left:50%;transform:translateX(-50%);background:var(--primary);color:white;padding:3px 14px;border-radius:20px;font-size:11px;font-weight:700;white-space:nowrap">✅ Plan actif</div>
      <?php elseif ($plan['populaire']??false): ?>
        <div style="position:absolute;top:-12px;left:50%;transform:translateX(-50%);background:var(--gold);color:white;padding:3px 14px;border-radius:20px;font-size:11px;font-weight:700;white-space:nowrap">⭐ Recommandé</div>
      <?php endif; ?>

      <div style="font-size:32px;margin-bottom:12px"><?= $plan['icone'] ?></div>
      ...
      <ul style="list-style:none;margin-bottom:24px">
        ...
        <li style="font-size:13px;padding:6px 0;border-bottom:1px solid var(--gris-100);display:flex;gap:8px">
          <span style="color:var(--primary)">✓</span> Archives officielles
        </li>
        <li style="font-size:13px;padding:6px 0;border-bottom:1px solid var(--gris-100);display:flex;gap:8px">
          <span style="color:var(--primary)">✓</span> Corrigés détaillés
        </li>
        <?php if ($plan['ia']): ?>
        <li style="font-size:13px;padding:6px 0;border-bottom:1px solid var(--gris-100);display:flex;gap:8px">
          <span style="color:var(--primary)">✓</span> Plan de révision IA
        </li>
        <?php endif; ?>
        <?php if (isset($plan['eleves_max'])): ?>
        <li style="font-size:13px;padding:6px 0;display:flex;gap:8px">
          <span style="color:var(--primary)">✓</span> <?= $plan['eleves_max'] ?> élèves
        </li>
        <?php endif; ?>
      </ul>

      <?php if ($isActive): ?>
        <div style="text-align:center;font-size:13px;color:var(--primary);font-weight:600">Plan actif ✅</div>
