<?php
/**
 * RÉUSSITE+ — Ajout d'explications aux questions QCM
...
echo "[" . date('H:i:s') . "] 📝 Ajout des explications aux questions...\n\n";

/* ═══════════════════════════════════════════
   MATHÉMATIQUES
═══════════════════════════════════════════ */
echo "[" . date('H:i:s') . "] 🔢 Mathématiques...\n";

add_expl($pdo, 'Quel est le résultat de 8 × 7 ?', 'A',
    '8 × 7 = 56. Astuce : 8×7 = 8×5 + 8×2 = 40 + 16 = 56. À retenir par cœur comme table de multiplication.');

add_expl($pdo, 'Quelle est la valeur de x dans : x + 12 = 20 ?', 'A',
    'Pour isoler x, on soustrait 12 des deux membres : x = 20 − 12 = 8. Vérification : 8 + 12 = 20 ✓');
...
add_expl($pdo, 'Résoudre : 2x - 5 = 11', 'A',
    '2x = 11 + 5 = 16, donc x = 16/2 = 8. Vérification : 2×8 − 5 = 16 − 5 = 11 ✓');
...
add_expl($pdo, 'Résoudre l\'équation du second degré : x² - 5x + 6 = 0', 'A',
    'Discriminant Δ = b²−4ac = 25−24 = 1. Racines : x = (5±1)/2. Donc x₁ = 3 et x₂ = 2. Vérification : (x−2)(x−3) = 0 ✓');
...
add_expl($pdo, 'Résoudre dans ℝ : |2x - 3| < 5', 'A',
    '|2x−3| < 5 ⟺ −5 < 2x−3 < 5 ⟺ −2 < 2x < 8 ⟺ −1 < x < 4.');

add_expl($pdo, 'Quel est le résultat de √144 ?', 'A',
    '√144 = 12 car 12² = 144. À connaître : √1=1, √4=2, √9=3, √16=4, √25=5, √36=6, √49=7, √64=8, √81=9, √100=10, √121=11, √144=12.');
...
add_expl($pdo, 'Exprimer 60° en radians.', 'A',
    '180° = π radians. Donc 60° = 60×(π/180) = π/3. Règle : multiplier les degrés par π/180.');
...
add_expl($pdo, 'Résoudre : x² - 5x + 6 = 0', 'A',
    'Δ = 25 − 24 = 1. x = (5±1)/2 → x=3 ou x=2. Ou par factorisation : (x−2)(x−3)=0.');
...
/* ═══════════════════════════════════════════
   FRANÇAIS
═══════════════════════════════════════════ */
echo "[" . date('H:i:s') . "] 📝 Français...\n";

add_expl($pdo, 'Quel est le pluriel de "œil" en français ?', 'A',
    '"yeux" est un pluriel irrégulier d\'origine latine. "Œils" n\'est utilisé que dans des termes composés comme "œils-de-bœuf" (fenêtres rondes).');
...
add_expl($pdo, '"Le cœur de Pierre est de pierre" — quelle figure de style ?', 'A',
    'L\'antanaclase répète un même mot avec deux sens différents : "cœur" (organe/siège des émotions) et "pierre" (matériau/dureté). C\'est un jeu sur la polysémie.');
...
add_expl($pdo, 'Identifier le COD dans : "Marie offre une fleur à son père."', 'A',
    'Le COD répond à la question "offre quoi ?". Réponse : "une fleur". "À son père" est le COI (complément d\'objet indirect).');
...
add_expl($pdo, 'La formule de l\'ammoniac est :', 'A',
    'L\'ammoniac NH₃ est une molécule composée d\'1 azote et de 3 hydrogènes. C\'est un gaz piquant soluble dans l\'eau, utilisé comme engrais et produit chimique industriel.');
...
add_expl($pdo, 'La formule de l\'acide nitrique est :', 'A',
    'L\'acide nitrique HNO₃ est un acide fort oxydant. Il est utilisé dans la fabrication d\'engrais (nitrate d\'ammonium) et d\'explosifs. À distinguer de HNO₂ (acide nitreux, acide faible).');
...
add_expl($pdo, 'Les vaisseaux sanguins qui transportent le sang vers le cœur sont :', 'A',
    'Les VEINES transportent le sang vers le cœur (V comme "vers"). Les ARTÈRES transportent le sang du cœur vers les organes. Les capillaires font les échanges entre le sang et les tissus.');
...
/* ═══════════════════════════════════════════
   HISTOIRE-GÉOGRAPHIE
═══════════════════════════════════════════ */
echo "[" . date('H:i:s') . "] 🌍 Histoire-Géo...\n";

add_expl($pdo, 'La conférence de Berlin (1884-1885) a abouti à :', 'A',
    'La conférence de Berlin (organisée par Bismarck) a organisé le "partage" de l\'Afrique entre les puissances européennes sans consulter les Africains. Elle a fixé les règles de la colonisation et créé l\'État Indépendant du Congo pour Léopold II de Belgique.');

add_expl($pdo, 'Qui était Patrice Lumumba ?', 'A',
    'Patrice Émery Lumumba fut le premier Premier ministre de la République du Congo indépendante (30 juin 1960). Nationaliste panafricain, il fut renversé et assassiné le 17 janvier 1961. Il est considéré comme un héros national.');
...
add_expl($pdo, 'La colonisation de la RDC par la Belgique a officiellement pris fin en :', 'A',
    'Le Congo belge a accédé à l\'indépendance le 30 juin 1960, proclamée par Patrice Lumumba. Avant d\'être belge (1908), il était l\'État Indépendant du Congo, propriété personnelle du roi Léopold II (1885-1908).');

add_expl($pdo, 'La ville de Lubumbashi est connue pour :', 'A',
    'Lubumbashi (ex-Élisabethville) est la 2ème ville de la RDC et la capitale du Haut-Katanga. Elle est au cœur de la "Copperbelt" (ceinture de cuivre) et riche en cuivre, cobalt et uranium. Le cobalt katangais est essentiel aux batteries de véhicules électriques.');

add_expl($pdo, 'Quel est le fleuve le plus long d\'Afrique ?', 'A',
    'Le Nil (6 650 km) est le plus long fleuve d\'Afrique et du monde (disputé avec l\'Amazone). Il coule du lac Victoria au nord jusqu\'à la mer Méditerranée en Égypte. Le Congo est le 2ème fleuve d\'Afrique par la longueur mais le premier par le débit.');

/* ═══════════════════════════════════════════
   ANGLAIS
═══════════════════════════════════════════ */
echo "[" . date('H:i:s') . "] 🇬🇧 Anglais...\n";
...
add_expl($pdo, 'La couche d\'ozone protège la Terre des rayons :', 'A',
    'La couche d\'ozone (O₃) dans la stratosphère absorbe 97 à 99% des rayons UV-B et UV-C du Soleil. Ces rayons peuvent causer des cancers de la peau et des cataractes. Elle est menacée par les CFC (chlorofluorocarbures).');

add_expl($pdo, 'La température d\'ébullition de l\'eau à pression normale :', 'A',
    'L\'eau bout à 100°C à la pression atmosphérique standard (1013 hPa). À altitude élevée (pression plus basse), l\'eau bout à moins de 100°C. Dans un autocuiseur (pression élevée), elle bout au-dessus de 100°C.');
...
add_expl($pdo, 'L\'effet de serre naturel est essentiel car il :', 'A',
    'Sans l\'effet de serre naturel, la température moyenne de la Terre serait de −18°C au lieu de +15°C. Les gaz à effet de serre (H₂O, CO₂, CH₄) retiennent la chaleur. Le problème actuel est l\'effet de serre AMPLIFIÉ par les activités humaines.');

/* ═══════════════════════════════════════════
   FIN — Résumé
═══════════════════════════════════════════ */
echo "\n[" . date('H:i:s') . "] ✅ Explications ajoutées : $updated questions mises à jour.\n";
echo "[" . date('H:i:s') . "] 💡 Les explications apparaissent maintenant lors de la révision des réponses.\n";
